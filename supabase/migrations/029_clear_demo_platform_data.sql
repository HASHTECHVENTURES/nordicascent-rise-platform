-- Clear test/demo platform data. Keeps admin account and pipeline config only.
-- Run once in Supabase SQL Editor when resetting for a clean production start.
-- WARNING: This deletes all candidates, companies, jobs, messages, and non-admin users.
...
-- One-click reset from Admin → Settings → Data (after this migration runs once).
...
BEGIN
  ...
  DELETE FROM public.stage_tasks;
  DELETE FROM public.profiles WHERE role <> 'admin';
  DELETE FROM auth.users WHERE email IS DISTINCT FROM 'admin@nordicascent.com';
END;
$$;
