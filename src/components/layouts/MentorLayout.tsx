...
const navigation = [
  ...
  { name: "Assigned candidates", href: "/mentor/candidates", icon: UsersRound },
];
      ...
      >
        <div className="flex h-full flex-col">
          <div className="flex h-14 items-center justify-between px-4 border-b border-border">
            {!collapsed && (
              <Link to="/mentor/dashboard" className="text-sm font-semibold text-foreground">
                Nordic Ascent
              </Link>
            )}
            {collapsed && (
              <Link to="/mentor/dashboard" className="mx-auto text-lg font-bold text-primary">
                N
              </Link>
            )}
            <Button
          ...
          {!collapsed && (
            <div className="p-4 border-t border-border space-y-2">
              <p className="text-sm font-medium truncate">{profile?.full_name ?? "Mentor"}</p>
      ...
      <div className={cn(collapsed ? "ml-16" : "ml-64")}>
        <header className="sticky top-0 z-30 h-16 bg-background border-b">
          <div className="flex h-full items-center px-6">
            <h1 className="text-lg font-medium text-foreground">Mentor programme</h1>
          </div>
        </header>
        <main className="p-6">
          <Outlet />
