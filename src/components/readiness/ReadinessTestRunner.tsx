...
export default function ReadinessTestRunner({ test, attempt }: Props) {
  ...
  const { data: questions, isLoading, isError, error } = useReadinessQuestions(test.id);
    ...
    <div className="space-y-6 max-w-3xl">
      {strictTimer && expiresAtMs && (
        <div className="sticky top-0 z-20 -mx-1 px-1 py-2 bg-background/95 backdrop-blur border-b border-border">
          <div className="flex flex-wrap items-center justify-between gap-3">
            ...
            <ReadinessCountdown expiresAtMs={expiresAtMs} hard size="lg" />
          </div>
        </div>
      )}
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex flex-wrap gap-3">
                    <Button
                      variant="outline"
                      size="sm"
                      disabled={timeExpired || uploading === q.id || submitAttempt.isPending || submittedRef.current}
                      asChild
                    >
                  ...
                  <div className="space-y-1">
                    ...
                    <Input
                      ...
                      disabled={timeExpired || submitAttempt.isPending || submittedRef.current}
                    />
                  </div>
                </div>
              ) : (
      ...
      <div className="sticky bottom-4 flex flex-wrap items-center justify-between gap-3 pb-4">
        ...
        <div className="flex gap-3 ml-auto">
          <Button variant="outline" onClick={() => navigate("/candidate/readiness")} disabled={timeExpired}>
            Save & exit
          </Button>
          <Button
            disabled={timeExpired || submitAttempt.isPending || submittedRef.current}
