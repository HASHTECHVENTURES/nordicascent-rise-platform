...
export default function AdminMentoringApplication() {
    ...
    return (
      <div className="space-y-4 max-w-lg py-12">
        ...
        <Button variant="outline" asChild>
          <Link to="/admin/mentoring">Back to Mentoring</Link>
        </Button>
      </div>
    );
  }
  ...
  if (!app.readiness_unlocked_at) {
    return (
      <div className="space-y-4 max-w-lg py-12">
        ...
        <Button variant="outline" asChild>
          <Link to={`/admin/selection/${app.id}`}>Open in Selection</Link>
        </Button>
      </div>
    );
  }
  ...
  const track =
    (app.track as Track | null) ?? ((app.candidates as { track?: Track } | null)?.track ?? "entry");

  return (
    <div className="space-y-6 max-w-3xl">
      <div className="flex items-start gap-4">
        <Button variant="ghost" size="icon" asChild>
          <Link to="/admin/mentoring">
