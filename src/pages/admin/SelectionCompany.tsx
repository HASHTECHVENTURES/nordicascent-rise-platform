...
import {
  useAdminCompanySelectionApplications,
  useBulkSelectionDecision,
} from "@/hooks/useSelection";
import { useAdminEmployers } from "@/hooks/useData";
...
const AdminSelectionCompany = () => {
  const { companyId } = useParams<{ companyId: string }>();
  const { data: companies } = useAdminEmployers();
  const { data: companyData, isLoading: appsLoading } = useAdminCompanySelectionApplications(companyId);
  ...
  const companyName =
    company?.name ??
    (companyData?.applications?.[0]?.jobs as { companies?: { name?: string } } | null)?.companies?.name ??
    "Company";

  useEffect(() => {
    setPage(1);
    setSelectedIds(new Set());
  }, [companyId, stepFilter, search, attention, sortMode, pageSize]);
  ...
  const allApps = companyData?.applications ?? [];
  ...
  const positions = jobs.reduce((sum, j) => sum + (j.positions_count ?? 0), 0);
  const maxSelected = maxSelectionsForJob(positions || 2);
    ...
    return (
      <div className="space-y-4 py-12">
        ...
        <Button variant="outline" asChild>
          <Link to="/admin/selection">Back to companies</Link>
        </Button>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-5 pb-24">
      ...
      <div className="flex flex-wrap items-center gap-x-4 gap-y-2 rounded-md border bg-muted/30 px-4 py-3 text-sm">
        <span className="font-medium">{jobs.length} job role{jobs.length === 1 ? "" : "s"}</span>
        <span className="text-muted-foreground">·</span>
        <span className="font-medium">{positions} positions</span>
        <span className="text-muted-foreground">·</span>
        <span>
          {selectedCount} selected
          {selectedCount >= maxSelected && positions > 0 && (
...
export default AdminSelectionCompany;
